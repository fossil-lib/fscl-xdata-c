...
XTEST_CASE(xdata_let_dlist_of_int_insert_and_get_values) {
    ...
    int num_values = trilo_xdata_dlist_get_values(dlist_data.dlist, &values);
...
XTEST_CASE(xdata_let_dlist_of_int_remove) {
    ...
    XASSERT_INT_EQUAL(1, trilo_xdata_tofu_get_integer(values[0]));
    XASSERT_INT_EQUAL(3, trilo_xdata_tofu_get_integer(values[1]));
...
XTEST_CASE(xdata_let_dlist_of_int_type_mismatch) {
    // Test inserting values of different types into the TriloDoublyList
    dlist_data.dlist = trilo_xdata_dlist_create(INTEGER_TYPE);
    ...
    XASSERT_INT_EQUAL(TRILO_XDATA_DLIST_TYPE_MISMATCH, trilo_xdata_dlist_insert(dlist_data.dlist, value2));
...
void xdata_test_dlist_group(XUnitRunner *runner) {
    ...
    XTEST_RUN_UNIT(xdata_let_dlist_of_int_type_mismatch,         runner);
} // end of func
